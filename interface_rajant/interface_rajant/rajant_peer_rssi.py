...
class PeerPublisher():
    ...
    def __init__(self, target_name, ros_node):
        ...
        self.ros_node.get_logger().info(
            f'{ros_node.this_robot} - Rajant Peer RSSI - Topic /{topic_name} created')
    ...
    def publish_all(self, ts):
        ...
        if ts != self.last_registered_timestamp:
            self.ros_node.get_logger().error(
                f'{self.ros_node.this_robot} - Rajant Peer RSSI'
                ' - Timestamp of end message different than'
                ' last registered timestamp in publish_all')
            return
        ...
        if not len(all_ts):
            self.ros_node.get_logger().error(
                f'{self.ros_node.this_robot} - Rajant Peer RSSI'
                ' - Empty list of timestamps in publish_all.')
            return
        if len(set(all_ts)) != 1:
            self.ros_node.get_logger().error(
                f'{self.ros_node.this_robot} - Rajant Peer RSSI'
                ' - Multiple different timestamps for the'
                ' same group in publish_all.')
            return
        ...
        if len(set(self.published_rssi_queue)) == 1 and \
            ...
            if self.repeated_counter < 4:
                ...
                self.ros_node.get_logger().debug(
                    f'{self.ros_node.this_robot} - Rajant Peer'
                    f' RSSI - Repeated RSSI for {self.target_name}'
                    f' for the last {self.QUEUE_LENGTH*3} seconds.'
                    f' Throttling counter {self.repeated_counter}')
                return

        self.ros_node.get_logger().debug(
            f'{self.ros_node.this_robot} - Rajant Peer RSSI - Publishing {self.target_name}')
        self.repeated_counter = 0
...
class RajantPeerRSSI(Node):
    def __init__(self):
        ...
        def signal_handler(sig, frame):
            if not self.shutdownTriggered.is_set():
                self.logger.warning(
                    f'{self.this_robot} - Rajant Peer RSSI - Got SIGINT. Triggering shutdown.')
                self.shutdown()
        ...
        self.robot_configs_file = self.get_parameter(
            'robot_configs').get_parameter_value().string_value
        try:
        ...
        self.radio_configs_file = self.get_parameter(
            'radio_configs').get_parameter_value().string_value
        try:
        ...
        self.bcapi_jar_file = self.get_parameter(
            'bcapi_jar_file').get_parameter_value().string_value
        if not (os.path.isfile(self.bcapi_jar_file) and
                self.bcapi_jar_file.endswith('.jar')):
            self.get_logger().error(
                f'{self.this_robot} - Rajant Peer RSSI - Erroneous BCAPI jar file')
            raise ValueError('Erroneous BCAPI file')
        ...
        if rajant_name in self.radio_configs.keys():
            ...
            self.get_logger().error(
                f'{self.this_robot} - Rajant Peer RSSI'
                f' - Radio {rajant_name} for robot'
                f' {self.this_robot} not found in configs')
            raise ValueError(
                f'Radio {rajant_name} for robot'
                f' {self.this_robot} not found in configs')
        ...
        if not ping(self.target_ip):
            self.get_logger().error(
                f'{self.this_robot} - Rajant Peer RSSI - Failed to ping {self.target_ip}')
    ...
    def enqueue_output(self):
        """Save the output of the process in a queue to be parsed afterwards."""
    ...
    def process_output(self):
        ...
        while not self.process_thread_shutdown.is_set():
            if self.enqueue_thread is not None and not self.enqueue_thread.is_alive():
                time.sleep(1)
                self.get_logger().error(
                    f'{self.this_robot} - Rajant Peer RSSI - Java process died, restarting')
                self.start_java_process_and_queue()
                restart_count += 1
                if restart_count == 5:
                    # shutdown
                    self.get_logger().error(
                        f'{self.this_robot} - Rajant Peer RSSI'
                        ' - Java process died too many times.'
                        ' Killing node.')
                    sys.exit(1)
                continue
            try:
    ...
    def start_java_process_and_queue(self):
        ...
        self.get_logger().info(
            f'{self.this_robot} - Rajant Peer RSSI'
            f' - Starting Java Process for IP {self.target_ip}')
        ...
        self.java_process = subprocess.Popen(
            ['java', '-jar', self.bcapi_jar_file, self.target_ip],
